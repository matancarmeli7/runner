...
def test_should_create_log_file():
    correct_log_file = '{}'.format(
        datetime.now().strftime('runner_test_%H_%M_%d_%m_%Y.log'))
...
def test_should_create_pcap_file():
    correct_pcap_file = '{}'.format(
        datetime.now().strftime('runner_test_%H_%M_%d_%m_%Y.pcap'))
...
def test_should_create_log_file_and_check_if_sys_trace_option_works():
    correct_log_file = '{}'.format(datetime.now().strftime(
        'runner_number_1_date_%H_%M_%d_%m_%Y.log'))
    runner.create_runner(
        'ls -l some_not_existing_file', 1, 0, True)
...
def test_should_create_log_file_and_check_if_call_trace_option_works():
    correct_log_file = '{}'.format(datetime.now().strftime(
        'runner_number_1_date_%H_%M_%d_%m_%Y.log'))
    runner.create_runner(
        'ls -l some_not_existing_file', 1, 0, False, True)
...
def test_should_create_log_file_and_check_if_log_trace_option_works():
    correct_log_file = '{}'.format(datetime.now().strftime(
        'runner_number_1_date_%H_%M_%d_%m_%Y.log'))
    runner.create_runner(
        'ls -l some_not_existing_file', 1, 0, False, False, True)
...
def test_should_create_pcap_file_and_check_if_net_trace_option_works():
    correct_pcap_file = '{}'.format(datetime.now().strftime(
        'runner_number_0_date_%H_%M_%d_%m_%Y.pcap'))
    runner.create_runner(
